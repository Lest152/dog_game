...
private:
    ...
    virtual void HandleRequest(HttpRequest&& request, std::string&& ip) = 0;

    virtual std::shared_ptr<SessionBase> GetSharedThis() = 0;

    // tcp_stream содержит внутри себя сокет и добавляет поддержку таумаутов
...
private:
    void HandleRequest(HttpRequest&& request, std::string&& ip) override {
        // Захватываем умный указатель на текущий объект Session в лямбде,
        // чтобы продлить время жизни сессии до вызова лямбды.
        // Используется generic-лямбда функция, способная принять response произвольного типа
        request_handler_(std::move(ip), std::move(request), [self = this->shared_from_this()](auto&& response) {
...
public:
    ...
    Listener(net::io_context& ioc, const tcp::endpoint& endpoint, Handler&& request_handler)
        : ioc_(ioc)
        // Обработчики асинхронных операция acceptor_ будут вызываться в своем strand
...
private:
    void DoAccept() {
        acceptor_.async_accept(
            // Передаем последовательный исполнитель, в котором будут вызываться обработчики
            // асинхронных операция сокета
            net::make_strand(ioc_),
            // С помощью bind_front_handler создаем обработчик, привязанный к методу OnAccept
            // текущего объекта.
